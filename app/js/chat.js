...
// --- AI CHAT SUMMARIZATION ENGINE (GROQ CLOUD AI) ---
let currentAISummaryRaw = '';

function formatMarkdownSummaryToHTML(md) {
    if (!md) return '';
    let html = escapeHTML(md);
    
    // Headers (### Header)
    html = html.replace(/###\s+(.+)/g, '<h3 style="margin-top: 1rem; margin-bottom: 0.4rem; font-size: 0.98rem; font-weight: 700; color: var(--text-main);">$1</h3>');
    html = html.replace(/##\s+(.+)/g, '<h2 style="margin-top: 1rem; margin-bottom: 0.4rem; font-size: 1.05rem; font-weight: 700; color: var(--text-main);">$1</h2>');
    ...
    lines.forEach(line => {
        ...
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
            if (!inList) {
                resultLines.push('<ul style="margin: 0 0 0.75rem 1.2rem; padding: 0;">');
                inList = true;
            }
            resultLines.push(`<li style="margin-bottom: 0.35rem; color: var(--text-muted);">${trimmed.substring(2)}</li>`);
        } else {
            if (inList) {
                resultLines.push('</ul>');
                inList = false;
            }
            if (trimmed.length > 0 && !trimmed.startsWith('<h3') && !trimmed.startsWith('<h2')) {
                resultLines.push(`<p style="margin: 0.3rem 0; color: var(--text-muted);">${line}</p>`);
            } else {
...
window.triggerChatSummarization = async function() {
    if (!targetUserId && !targetRoomId) {
        showToast("Please select a conversation or community room first to summarize.", "info");
        return;
    }

    const modal = document.getElementById('ai-summary-modal');
    const titleEl = document.getElementById('ai-summary-chat-title');
    const loader = document.getElementById('ai-summary-loader');
    const errorBox = document.getElementById('ai-summary-error');
    const errorText = document.getElementById('ai-summary-error-text');
    const contentBox = document.getElementById('ai-summary-content');
    const textTarget = document.getElementById('ai-summary-text-target');
    const msgCountEl = document.getElementById('ai-summary-msg-count');

    if (!modal) return;

    // Set title
    let displayTitle = 'Chat Summary';
    if (targetRoomId) {
        displayTitle = `✨ Summary: ${escapeHTML(activeRoomName || 'Group')}`;
    } else if (targetUserId) {
        const titleHeader = document.getElementById('chat-window-title');
        const userTitle = titleHeader ? titleHeader.innerText : 'Direct Chat';
        displayTitle = `✨ Summary: ${escapeHTML(userTitle)}`;
    }
    ...
    try {
        const res = await fetch('/api/chat/summarize', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ targetUserId, targetRoomId })
        });
        ...
        if (res.ok && data.success) {
            currentAISummaryRaw = data.summary;
            if (msgCountEl) msgCountEl.innerText = `Summarized ${data.messageCount} messages`;
            if (textTarget) textTarget.innerHTML = formatMarkdownSummaryToHTML(data.summary);
            if (contentBox) contentBox.classList.remove('hidden');
        } else {
            if (errorText) errorText.innerText = data.error || 'Failed to generate summary.';
            if (errorBox) errorBox.classList.remove('hidden');
        }
    } catch (err) {
        console.error('AI Summarization fetch error:', err);
        if (loader) loader.classList.add('hidden');
        if (errorText) errorText.innerText = 'Network connection failure while communicating with AI service.';
        if (errorBox) errorBox.classList.remove('hidden');
    }
};

window.copyAISummaryToClipboard = async function() {
    if (!currentAISummaryRaw) return;
    try {
        await navigator.clipboard.writeText(currentAISummaryRaw);
        const btnText = document.getElementById('copy-summary-btn-text');
        if (btnText) {
            btnText.innerText = 'Copied! ✅';
            setTimeout(() => { btnText.innerText = 'Copy Summary'; }, 2000);
        }
